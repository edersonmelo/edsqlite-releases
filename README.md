...
**Dá para usar, mas ainda não é uma versão estável.** A 0.10 serve para experimentar, aprender e
...
```bash
tar xzf edsqlite-0.10.0-macos-arm64.tar.gz
...
restrições (`NOT NULL`, `DEFAULT`, `INTEGER PRIMARY KEY`), `LEFT JOIN`, `UNION`, `ALTER TABLE`,
